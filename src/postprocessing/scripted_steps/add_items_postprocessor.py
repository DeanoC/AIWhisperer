...
def add_items_postprocessor(content: Union[str, dict, list], data: dict) -> Tuple[Union[str, dict, list], Dict]:
    ...
    step_result = {"success": True, "changes": [], "errors": [], "warnings": [], "logs": []}
    ...
    if original_type_was_str:
        if not content.strip():
            ...
            try:
                ...
                logger.debug(
                    f"add_items_postprocessor returning original content due to JSONDecodeError. Data keys: {data.keys()}"
                )
                return (content, data)
            except Exception as e:
                err_msg = f"An unexpected error occurred during JSON parsing: {e}"
                logger.debug(
                    f"add_items_postprocessor returning original content due to unexpected error. Data keys: {data.keys()}"
                )
    ...
    if isinstance(parsed_content, dict) and top_level_items:
        ...
        step_result["warnings"].append(
            f"Top-level items specified, but content is not a dictionary (type: {type(parsed_content).__name__})"
        )
    ...
    if step_level_items:
        ...
        def find_and_add_to_steps(obj, container_key, items_to_add_dict):
            ...
            if isinstance(obj, dict):
                for key, value in obj.items():
                    if key == container_key:
                        if isinstance(value, list):
                            logger.debug(f"Found step container '{container_key}' as a list under key '{key}'.")
                            for item in value:
                            ...
                            warn_msg = f"Found key '{container_key}' but it is not a list (type: {type(value).__name__}), cannot add step-level items here."
                            logger.warning(warn_msg)
                            step_result["warnings"].append(warn_msg)
                ...
                logger.debug(f"Skipping unsupported type in recursive search: {type(obj).__name__}")
            return obj
        ...
        step_level_changes_count = sum(
            1 for change in step_result["changes"][initial_changes_count:] if "step-level item" in change
        )

        if step_level_changes_count == 0:
            # Check if there were any warnings about the container key being found but not being a list
            container_key_found_but_not_list = any(
                f"Found key '{step_container_key}' but it is not a list" in warning
                for warning in step_result["warnings"]
            )
            if not container_key_found_but_not_list:
                step_result["warnings"].append(
                    f"Step container '{step_container_key}' not found or not a list of dictionaries."
                )
    ...
    if original_type_was_str and isinstance(parsed_content, (dict, list)) and step_result["success"]:
        ...
        try:
            ...
            logger.debug(
                f"add_items_postprocessor returning processed content (dict/list) due to dump error. Data keys: {data.keys()}"
            )
...
if __name__ == "__main__":
    ...
    test_cases = [
        (
            '{"title": "Test", "plan": [{"subtask_id": "1"}]}',
            {"items_to_add": {"top_level": {"task_id": "abc"}, "step_level": {"status": "new"}}},
            "Add top and step level",
        ),
        ("{}", {"items_to_add": {"top_level": {"task_id": "abc"}}}, "Add to empty string"),
        ("   ", {"items_to_add": {"top_level": {"task_id": "abc"}}}, "Add to whitespace string"),
        (
            '{"title": "Test"}',
            {"items_to_add": {"step_level": {"status": "new"}}},
            "Add step level to dict without container",
        ),
        (
            '{"title": "Test", "plan": "not a list"}',
            {"items_to_add": {"step_level": {"status": "new"}}},
            "Step container not a list",
        ),
        (
            '{"title": "Test", "plan": [{"subtask_id": "1", "status": "done"}]}',
            {"items_to_add": {"step_level": {"status": "new"}}},
            "Add step level with existing key",
        ),
        (
            '{"title": "Test", "subtasks": [{"subtask_id": "1"}]}',
            {"items_to_add": {"step_level": {"status": "new"}, "step_container": "subtasks"}},
            "Add step level to custom container",
        ),
        ('{"key": "value"', {"items_to_add": {"top_level": {"task_id": "abc"}}}, "Invalid JSON string"),
        (
            {"title": "Test Dict", "plan": [{"subtask_id": "1"}]},
            {"items_to_add": {"top_level": {"task_id": "abc"}, "step_level": {"status": "new"}}},
            "Dict input",
        ),
        ([{"subtask_id": "1"}], {"items_to_add": {"top_level": {"task_id": "abc"}}}, "List input (no top level add)"),
        (
            '{"tasks": [{"steps": [{"subtask_id": "1"}]}]}',
            {"items_to_add": {"step_level": {"status": "new"}, "step_container": "steps"}},
            "Nested step container",
        ),
        (
            '{"tasks": [{"steps": "not a list"}]}',
            {"items_to_add": {"step_level": {"status": "new"}, "step_container": "steps"}},
            "Nested step container not a list",
        ),
        (
            '{"tasks": [{"steps": [{"subtask_id": "1"}, "not a dict"]}]}',
            {"items_to_add": {"step_level": {"status": "new"}, "step_container": "steps"}},
            "Step container with non-dict items",
        ),
    ]
    ...
    for i, (test_content, test_data, desc) in enumerate(test_cases):
        ...
        try:
            ...
            print(
                "Logs:",
                json.dumps(output_data.get("steps", {}).get("add_items_postprocessor", {}).get("logs", []), indent=2),
            )
            if output_data.get("steps", {}).get("add_items_postprocessor", {}).get("errors"):
                print(
                    "Errors:",
                    json.dumps(
                        output_data.get("steps", {}).get("add_items_postprocessor", {}).get("errors", []), indent=2
                    ),
                )
            if output_data.get("steps", {}).get("add_items_postprocessor", {}).get("warnings"):
                print(
                    "Warnings:",
                    json.dumps(
                        output_data.get("steps", {}).get("add_items_postprocessor", {}).get("warnings", []), indent=2
                    ),
                )
            if output_data.get("steps", {}).get("add_items_postprocessor", {}).get("changes"):
                print(
                    "Changes:",
                    json.dumps(
                        output_data.get("steps", {}).get("add_items_postprocessor", {}).get("changes", []), indent=2
                    ),
                )
