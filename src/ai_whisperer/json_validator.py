...
if __name__ == '__main__':
    ...
    now_ts_str = format_timestamp()
    print(f"Formatted current timestamp: {now_ts_str}")
    assert "Z" in now_ts_str or "+" in now_ts_str # Check for UTC indicator

    specific_dt = datetime(2023, 1, 15, 10, 30, 0, tzinfo=timezone.utc)
    specific_ts_str = format_timestamp(specific_dt)
    print(f"Formatted specific timestamp: {specific_ts_str}")
    assert specific_ts_str == "2023-01-15T10:30:00+00:00"

    print("\n--- Testing Timestamp Parsing ---")
    parsed_dt = parse_timestamp(now_ts_str)
    print(f"Parsed timestamp back to datetime: {parsed_dt}")
    ...
    if task_plan_schema and subtask_schema:
        print("\n--- Testing Task Plan Validation (Example) ---")
        valid_task_plan = {
            "task_id": generate_uuid(),
            "name": "Test Task Plan",
            "description": "A plan for testing.",
            "goal": "Ensure validation works.",
            "status": "pending",
            "created_at": format_timestamp(),
            "updated_at": format_timestamp(),
            "subtasks": [generate_uuid()],
            "metadata": {"priority": "high"}
        }
        is_valid, error = validate_task_plan(valid_task_plan)
        print(f"Valid Task Plan - Valid: {is_valid}, Error: {error}")
        assert is_valid

        invalid_task_plan = {
            "task_id": "not-a-uuid", # Invalid format
            "name": "Invalid Task",
            # Missing required fields like description, goal, status, etc.
        }
        is_valid, error = validate_task_plan(invalid_task_plan)
        print(f"Invalid Task Plan - Valid: {is_valid}, Error: {error}")
        assert not is_valid
        print(f"Validation error message: {error}")


        print("\n--- Testing Subtask Validation (Example) ---")
        valid_subtask = {
            "subtask_id": generate_uuid(),
            "task_id": generate_uuid(),
            "name": "Test Subtask",
            "description": "A subtask for testing.",
            "instructions": "Do this test.",
            "status": "in_progress",
            "created_at": format_timestamp(),
            "updated_at": format_timestamp(),
            "output": "Not yet completed.",
            "metadata": {"complexity": "low"}
        }
        is_valid, error = validate_subtask(valid_subtask)
        print(f"Valid Subtask - Valid: {is_valid}, Error: {error}")
        assert is_valid
        ...
        invalid_subtask = {
            ...
            "description": "Invalid subtask due to name type.",
            "instructions": "...",
            "status": "pending",
            "created_at": "not-a-date", # Invalid format
            "updated_at": format_timestamp()
        }
        is_valid, error = validate_subtask(invalid_subtask)
        print(f"Invalid Subtask - Valid: {is_valid}, Error: {error}")
        assert not is_valid
        print(f"Validation error message: {error}")
    else:
        print("\n--- Skipping schema validation tests as schemas failed to load ---")

    print("\nAll basic tests in json_validator.py completed.")