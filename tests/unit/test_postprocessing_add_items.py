...
"""
    ...
    # Act
    (modified_json_string, updated_result) = add_items_postprocessor(
...
"""
    result_data = {"items_to_add": {"step_level": {"subtask_id": "xyz-789"}}}
...
"""
  ...
  def test_empty_json_string(self):
    ...
    # Arrange
    json_string = ""
    result_data = {"items_to_add": {"top_level": {"task_id": "abc-123"}}}
    ...
    assert "add_items_postprocessor" in updated_result["steps"]
    assert any(
      "Input string is empty or whitespace-only, treating as empty object." in log
      for log in updated_result["steps"]["add_items_postprocessor"]["logs"]
    )
    assert any(
      "Using empty dictionary due to empty input string" in warning
      for warning in updated_result["steps"]["add_items_postprocessor"][
        "warnings"
      ]
    )
    assert any(
      "Added/Overwrote top-level item: task_id" in change
      for change in updated_result["steps"]["add_items_postprocessor"]["changes"]
    )
  ...
  def test_invalid_json_string(self):
    ...
    # Assert
    assert (
      updated_result["steps"]["add_items_postprocessor"]["success"] is False
    ...
    assert (
      ...
      in updated_result["steps"]["add_items_postprocessor"]["errors"][0]
    )
...
"""
    ...
    assert "add_items_postprocessor" in updated_result["steps"]
    assert (
      "No items to add specified"
      in updated_result["steps"]["add_items_postprocessor"]["logs"][0]
    )
...
"""
    result_data = {
      "items_to_add": {
        "top_level": {"task_id": "new-id", "input_hashes": {"file1": "hash1"}}
      }
    }
    ...
    assert modified_json_dict["input_hashes"] == {
      "file1": "hash1"
    }  # New item added
    assert updated_result["steps"]["add_items_postprocessor"]["success"] is True
    assert "add_items_postprocessor" in updated_result["steps"]
...
"""
    result_data = {
      "items_to_add": {"step_level": {"subtask_id": "xyz-789"}},
    ...
    modified_json_dict = json.loads(
      modified_json_string
    )  # Should still be able to parse the original JSON

    # Assert - should warn about missing container but still succeed
    assert modified_json_dict == json.loads(
      json_string
    ...
    assert (
      "Step container 'nonexistent_container' not found or not a list"
      in updated_result["steps"]["add_items_postprocessor"]["warnings"][0]
    )
...
"""
    ...
    assert len(updated_result["steps"]["add_items_postprocessor"]["warnings"]) > 0
    assert (
      "Found key 'plan' but it is not a list (type: dict), cannot add step-level items here."
      in updated_result["steps"]["add_items_postprocessor"]["warnings"][0]
    )
    @pytest.mark.xfail(reason="Known issue with handling nested non-list items")
    def test_add_items_to_json_string_with_step_container_nested_under_non_list_items(
      self,
    ):
      ...
      """
      result_data = {
        "items_to_add": {"step_level": {"status": "completed"}},
      ...
      expected_json_dict = {
        "data": {
          ...
          "item2": {
            "id": "item2",
            "steps": [{"subtask_id": "step1.1", "status": "completed"}],
          },
        }
      }
      ...
      assert "add_items_postprocessor" in updated_result["steps"]
      assert any(
        "Successfully parsed content as JSON." in log
        for log in updated_result["steps"]["add_items_postprocessor"]["logs"]
      )
      assert any(
        "Added step-level item: status" in change
        for change in updated_result["steps"]["add_items_postprocessor"]["changes"]
      )
  ...
  @pytest.mark.xfail(reason="Known issue with handling dictionary input")
  def test_input_is_dictionary(self):
    ...
    input_dict = {
      "title": "Test Document",
      "description": "A test document",
      "plan": [{"subtask_id": "step1", "description": "First step"}],
    }
    result_data = {
      "items_to_add": {
        "top_level": {
    ...
    # Act
    (modified_content, updated_result) = add_items_postprocessor(
      input_dict, result_data
    )
    ...
    assert "add_items_postprocessor" in updated_result["steps"]
    assert any(
      "Input is a dict, processing directly." in log
      for log in updated_result["steps"]["add_items_postprocessor"]["logs"]
    )
    assert any(
      "Added/Overwrote top-level item: task_id" in change
    ...
    assert any(
      "Added step-level item: subtask_id" in change
      for change in updated_result["steps"]["add_items_postprocessor"]["changes"]
    )
  ...
  def test_input_is_list(self):
    ...
    # Arrange
    input_list = [{"subtask_id": "step1", "description": "First step"}]
    result_data = {"items_to_add": {"top_level": {"task_id": "abc-123"}}}

    # Act
    (modified_content, updated_result) = add_items_postprocessor(
      input_list, result_data
    )
    ...
    assert "add_items_postprocessor" in updated_result["steps"]
    assert any(
      "Input is a list, processing directly." in log
      for log in updated_result["steps"]["add_items_postprocessor"]["logs"]
    )
    assert not updated_result["steps"]["add_items_postprocessor"][
      "changes"
...
"""
    result_data = {"items_to_add": {"top_level": {"task_id": "abc-123"}}}
    expected_json_dict = {"name": "José", "city": "São Paulo", "task_id": "abc-123"}
...
"""
    result_data = {"items_to_add": {"top_level": {"task_id": "abc-123"}}}
    expected_json_dict = {"key1": "value1", "key2": None, "task_id": "abc-123"}
...
"""
    result_data = {"items_to_add": {"top_level": {"task_id": "abc-123"}}}
    expected_json_dict = {"key1": True, "key2": False, "task_id": "abc-123"}
...
"""
    result_data = {"items_to_add": {"top_level": {"task_id": "abc-123"}}}
    expected_json_dict = {"key1": 123, "key2": 45.67, "task_id": "abc-123"}
...
"""
    result_data = {"items_to_add": {"top_level": {"task_id": "abc-123"}}}
    expected_json_dict = {"key1": {}, "key2": [], "task_id": "abc-123"}
  ...
  @pytest.mark.xfail(reason="Dictionary input not handled correctly")
  ...
  "tasks": [
    {
      ...
      "steps": [
        {"subtask_id": "step1.1", "action": "Code"},
        {"subtask_id": "step1.2", "action": "Test"}
      ]
    },
    {
      "id": "task2",
      ...
      "steps": [
        {"subtask_id": "step2.1", "action": "Debug"}
      ]
    }
  ]
}
"""
    result_data = {
      "items_to_add": {
        "top_level": {"status": "in_progress"},
        "step_level": {"assigned_to": "Roo"},
    ...
    assert any(
      "Added/Overwrote top-level item: status" in change
      for change in updated_result["steps"]["add_items_postprocessor"]["changes"]
    )
    assert any(
      "Added step-level item: assigned_to" in change
      for change in updated_result["steps"]["add_items_postprocessor"]["changes"]
    )
    # Check that the change count is correct (3 steps modified)
    assert (
      len(updated_result["steps"]["add_items_postprocessor"]["changes"]) == 4
    )  # 1 top-level + 3 step-level