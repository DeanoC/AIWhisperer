...
class TestOpenRouterAPIUnit:
    ...
    def test_call_chat_completion_with_history(self, mock_post, api_client):
        ...
        prompt = "New message" # This should be ignored
        model = "test_model"
        params = {"temperature": 0.5}
        ...
        mock_post.assert_called_once_with(
            ...
            json={
                "model": model,
                "messages": history, # Should use history
                "temperature": 0.5
            },
            timeout=10
        )
    ...
    def test_stream_chat_completion_initial_connection_error(self, mock_post, api_client):
        ...
        with pytest.raises(OpenRouterConnectionError, match="Network error during OpenRouter API streaming: Network unreachable"):
            list(api_client.stream_chat_completion("prompt", "model", {}))
    ...
    def test_stream_chat_completion_mid_stream_error(self, mock_post, api_client):
        ...
        assert first_chunk == {"id":"chatcmpl-abc","choices":[{"index":0,"delta":{"role":"assistant","content":"Hello"}}]}

        # The next iteration should raise the error
        with pytest.raises(OpenRouterConnectionError, match="Network error during OpenRouter API streaming: Mid-stream network error"):
    ...
    def test_stream_chat_completion_invalid_json_chunk(self, mock_post, api_client):
        ...
        with pytest.raises(OpenRouterAPIError, match="Failed to decode JSON chunk from stream: Expecting value: line 1 column 1 \\(char 0\\). Chunk: invalid json..."):
            next(stream_generator)
