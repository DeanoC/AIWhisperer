...
def test_clean_backtick_wrapper(input_yaml, expected_yaml):
    ...
    # Call the function
    output_yaml, updated_result = clean_backtick_wrapper(input_yaml, result_data)
    ...
    assert "Removed backtick wrappers" in updated_result["logs"][-1]
...
```""")
    ...
    try:
        ...
        # Check that the backtick wrappers were removed
        assert not output_yaml.startswith("```")
        assert not output_yaml.endswith("```")

        # Check that the content is preserved
        assert "natural_language_goal: Enhance the OpenRouter" in output_yaml
        assert "plan:" in output_yaml
        assert "step_id: planning_cli_and_api_changes" in output_yaml

        # Check that backticks within the content are preserved
        assert "`--list-models`" in output_yaml
        assert "`--output-csv`" in output_yaml
