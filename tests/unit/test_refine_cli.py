    ...
    for filename in matching_files:
        try:
            ...
            if len(parts) == 2:
                ...
                number_part = number_part_with_suffix.rsplit(".", 1)[
                    0
                ]  # Split from right to handle dots in stem if any
...
def test_rename_first_iteration(mock_path_constructor, mock_fnmatch, mock_os_listdir):
    ...
    mock_fnmatch.side_effect = (
        lambda name, pattern: name.startswith("input_iteration")
        and name.endswith(".md")
        and pattern == "input_iteration*.md"
    )
...
def test_rename_subsequent_iteration(mock_path_constructor, mock_fnmatch, mock_os_listdir):
    ...
    mock_fnmatch.side_effect = (
        lambda name, pattern: name in ["input_iteration1.md", "input_iteration2.md"]
        and pattern == "input_iteration*.md"
    )
...
def test_rename_with_different_suffix(mock_path_constructor, mock_fnmatch, mock_os_listdir):
    ...
    mock_fnmatch.side_effect = (
        lambda name, pattern: name == "input_iteration1.txt" and pattern == "input_iteration*.txt"
    )
