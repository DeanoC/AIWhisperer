"""
Unit tests for Debbie agent configuration in conversation replay mode.
...
class TestDebbieAgentConfig:
    ...
    def test_debbie_agent_has_required_properties(self, debbie_agent):
        ...
        assert debbie_agent.role == "debugging_assistant, conversation_replayer"
        assert debbie_agent.prompt_file == "debbie_debugger.prompt.md"
        assert debbie_agent.description
    
    def test_debbie_agent_role_includes_conversation_replayer(self, debbie_agent):
        """Test that Debbie's role includes conversation replay"""
        assert "conversation_replayer" in debbie_agent.role
    
    def test_debbie_agent_has_conversation_tools(self, debbie_agent):
        """Test that Debbie has tools needed for conversation replay"""
        # Check for conversation-specific tool tags
        assert "conversation" in debbie_agent.tool_tags or "monitoring" in debbie_agent.tool_tags
    ...
    def test_debbie_agent_prompt_supports_conversation_mode(self, debbie_agent):
    ...
    def test_debbie_agent_dual_role_configuration(self, debbie_agent):
        """Test that Debbie can function as both debugger and conversation replayer"""
        roles = [r.strip() for r in debbie_agent.role.split(',')]
        assert "debugging_assistant" in roles
        assert "conversation_replayer" in roles