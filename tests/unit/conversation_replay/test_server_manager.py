"""
Unit tests for ai_whisperer.extensions.conversation_replay.server_manager
...
class TestServerManagerPortAllocation:
    ...
    @patch('ai_whisperer.extensions.conversation_replay.server_manager.random.randint')
...
class TestServerManagerWaitForReady:
    ...
    @patch('ai_whisperer.extensions.conversation_replay.server_manager.socket.socket')
    @patch('ai_whisperer.extensions.conversation_replay.server_manager.time.sleep')
    ...
    @patch('ai_whisperer.extensions.conversation_replay.server_manager.socket.socket')
    @patch('ai_whisperer.extensions.conversation_replay.server_manager.time.sleep')
    @patch('ai_whisperer.extensions.conversation_replay.server_manager.time.time')
    ...
    def test_wait_for_server_ready_exception_handling(self, mock_sleep, mock_socket_class):
        ...
        with patch('ai_whisperer.extensions.conversation_replay.server_manager.time.time', side_effect=[0, 15]):
            result = manager._wait_for_server_ready(timeout_seconds=10)
...
class TestServerManagerStartSubprocess:
    ...
    @patch('ai_whisperer.extensions.conversation_replay.server_manager.subprocess.Popen')
    ...
    def test_start_server_success_first_attempt(self, mock_randint):
        ...
        with patch.object(manager, '_start_subprocess') as mock_start_sub:
            with patch.object(manager, 'is_running', return_value=True):
                with patch('ai_whisperer.extensions.conversation_replay.server_manager.time.sleep'):
...
class TestServerManagerErrorHandling:
    ...
    def test_wait_for_server_ready_socket_creation_failure(self):
        ...
        with patch('ai_whisperer.extensions.conversation_replay.server_manager.socket.socket', side_effect=OSError("Socket creation failed")):
            with patch('ai_whisperer.extensions.conversation_replay.server_manager.time.time', side_effect=[0, 15]):
                result = manager._wait_for_server_ready(timeout_seconds=10)
                assert result is False
