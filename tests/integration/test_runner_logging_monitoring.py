...
def create_simple_test_plan():
    ...
    plan = {
        ...
        "steps": [
            {
                "step_id": "step_1_start",
                "description": "Starting step",
                "agent_spec": {"type": "dummy_start"}
            },
            {
...
def test_runner_logs_and_state_updates_on_step_execution(
    MockExecutionEngine, MockStateManager, caplog_aiwhisperer
):
    """
    ...
    mock_state_manager_instance.get_plan_state.return_value = PlanState(
        ...
        status="Running",
        steps=[
            StepState(step_id="step_1_start", status="Completed"),
            StepState(step_id="step_2_process", status="Running"),
            StepState(step_id="step_3_end", status="Pending")
        ]
    )
    mock_state_manager_instance.get_step_state.side_effect = lambda step_id: next(
        (s for s in mock_state_manager_instance.get_plan_state.return_value.steps if s.step_id == step_id), None
    )
    ...
    # Simulate step start
    step_id_to_test = "step_2_process"
    mock_state_manager_instance.update_step_state(step_id_to_test, "Running")
    ...
    logging.getLogger("ai_whisperer").info(
        LogMessage(
            ...
            event_summary=f"Starting execution of step '{step_id_to_test}'.", # Use event_summary
            step_id=step_id_to_test,
    ...
    mock_state_manager_instance.update_step_state(step_id_to_test, "Completed")
    ...
    logging.getLogger("ai_whisperer").info(
        LogMessage(
            ...
            event_summary=f"Step '{step_id_to_test}' completed successfully.", # Use event_summary
            step_id=step_id_to_test,
    ...
    mock_state_manager_instance.update_step_state.assert_any_call(step_id_to_test, "Running")
    mock_state_manager_instance.update_step_state.assert_any_call(step_id_to_test, "Completed")
    ...
    assert any(f"Starting execution of step '{step_id_to_test}'" in msg for msg in log_messages)
    assert any("Sent request to LLM." in msg for msg in log_messages)
    assert any(f"Step '{step_id_to_test}' completed successfully." in msg for msg in log_messages)
    ...
    step_start_log = next(
        (record for record in caplog_aiwhisperer.records if f"Starting execution of step '{step_id_to_test}'" in record.message),
        None
    )
    ...
    ai_interaction_log = next(
        (record for record in caplog_aiwhisperer.records if "Sent request to LLM." in record.message),
        None
    )
    ...
    step_completion_log = next(
        (record for record in caplog_aiwhisperer.records if f"Step '{step_id_to_test}' completed successfully." in record.message),
        None
    )
...
@patch('src.ai_whisperer.execution_engine.ExecutionEngine')
@patch('src.ai_whisperer.state_management.StateManager')
def test_runner_logs_and_state_updates_on_user_interaction(
    MockExecutionEngine, MockStateManager, caplog_aiwhisperer
):
    """
    ...
    mock_state_manager_instance.get_plan_state.return_value = PlanState(
        ...
        status="Running",
        steps=[
            StepState(step_id="step_to_pause", status="Running"),
        ]
    )
    mock_state_manager_instance.get_step_state.side_effect = lambda step_id: next(
        (s for s in mock_state_manager_instance.get_plan_state.return_value.steps if s.step_id == step_id), None
    ...
    step_id_to_interact = "step_to_pause"
    ...
    mock_state_manager_instance.update_step_state(step_id_to_interact, "Paused")
    logging.getLogger("ai_whisperer").info(
        LogMessage(
            level=LogLevel.INFO,
    ...
    mock_state_manager_instance.update_step_state(step_id_to_interact, "Running")
    logging.getLogger("ai_whisperer").info(
        LogMessage(
            level=LogLevel.INFO,
            component=ComponentType.USER_INTERACTION,
    ...
    mock_state_manager_instance.update_step_state(step_id_to_interact, "Cancelled")
    logging.getLogger("ai_whisperer").info(
        LogMessage(
            level=LogLevel.INFO,
            component=ComponentType.USER_INTERACTION,
    ...
    mock_state_manager_instance.update_step_state.assert_any_call(step_id_to_interact, "Paused")
    mock_state_manager_instance.update_plan_state.assert_any_call("Running")
    mock_state_manager_instance.update_step_state.assert_any_call(step_id_to_interact, "Running")
    mock_state_manager_instance.update_plan_state.assert_any_call("Cancelled")
    mock_state_manager_instance.update_step_state.assert_any_call(step_id_to_interact, "Cancelled")
    ...
    context_log = next(
        (record for record in caplog_aiwhisperer.records if "User provided additional context." in record.message),
        None
    )
    assert context_log is not None
    assert context_log.levelname == 'INFO'
    # Further checks on 'extra' dict for component, action, step_id, state_before, state_after, details
