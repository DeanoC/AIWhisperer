"""
Integration tests for Debbie agent in conversation replay mode.
...
class TestDebbieAgentIntegration:
    ...
    def test_debbie_continuation_config_includes_batch(self, agent_registry):
        ...
        # Find conversation-related rules
        conversation_rules = [r for r in rules if any(
            tool in r.get('trigger_tools', []) 
            for tool in ['conversation_processor', 'conversation_client']
        )]
        
        assert len(conversation_rules) >= 2, "Should have continuation rules for conversation tools"
